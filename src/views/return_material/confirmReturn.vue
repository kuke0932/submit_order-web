<template>
  <div style="height:100%;">
    <view-box ref="viewBox">
      <x-header :left-options="{preventGoBack: true}" @on-click-back="back"  slot="header">新增还料
      </x-header>
      <div class="container">
        <div class="infor_list" :style="'height:' + (viewHeight - 46) + 'px'">
                    ...
                    <flexbox-item class="details_right">
                      ...
                      <flexbox>
                        ...
                        <flexbox-item class="count">
                          ...
                          <input class="input_counts" type="number" style="border-bottom: 1px solid #ccc"  @blur="validInput(index)" v-model="material.MaterialNumber" @change="checkCount">
      ...
      save() {
        ...
        return materialDocumentsSave(data).then(resp => {
          if (resp.ret.Result.ResponseStatus.IsSuccess === 'True') {
            Promise.all([this.updateBorrow(), this.submit(resp.ret.Result.ResponseStatus.SuccessEntitys[0].Number)]).then(resps => {
              if (resps[0].Result.ResponseStatus.IsSuccess && resps[1].Result.ResponseStatus.IsSuccess) {
                const _this = this
                this.$vux.toast.show({
                  text: '保存成功!',
                  onHide() {
                    _this.$router.push({path: '/return_material/isReturn', query: {t: 3}})
                  }
                })
              } else {
        ...
        for (let i = 0; i < this.materialList.length; i++) {
          model.push({
            ...
            'FBillEntry': [{
              ...
              FBASEJOINQTY: Number.parseFloat(this.materialList[i].MaterialNumber),
              'FRECEIVEQTY': Number.parseFloat(this.materialList[i].MaterialNumber)
