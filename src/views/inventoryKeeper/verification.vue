<template>
  <div>
    <div style="position: fixed; top: 46px; width: 100%;z-index: 999">
      ...
      <div class="select_all" style="position: fixed; top: 170px; width: 100%;">
        <x-button @click.native="checkAll" mini :type="isAllSel == true ? 'default' : 'default'">全选/取消</x-button>
        <x-button style="margin-top: 0;" mini @click.native="batchUnAudit">反审核</x-button>
        <x-button style="margin-top: 0;" mini @click.native="deleteBill">删除</x-button>
      </div>
    </div>
    <div :style="'margin-top: 210px; width: 100%;height:'+ (viewHeight - 261) + 'px'">
      <scroller ref="scroller" style="position:relative;" :on-refresh="refresh" :on-infinite="infinite">
        <div class="infor_list">
          <div class="infor_item" v-for="(item, index) in (isSearch ? searchList : dataList)" v-bind:key="index">
              ...
              <flexbox-item class="details_right">
                <router-link :to="{name:'inventoryKeeperDetails', params: {borrowDetails: item, fromType: 0}}">
                  ...
                  <flexbox>
                    ...
                    <flexbox-item>状态：{{item.DocumentStatus == 'D'  ? '反审核' : '已审核'}}</flexbox-item>
                  </flexbox>
                </router-link>
              </flexbox-item>
            </flexbox>
...
<script>
  ...
  import { unAudit, removeReturn } from '@/api/base'
  import Vue from 'vue'
  ...
  export default {
    ...
    methods: {
      ...
      infiniteHandler($state) {
        ...
        this.fetchData(this.userName, this.date[0] || '', this.date[1] || '', '', pageIndex, pageSize).then(resp => {
          if (resp && resp.length) {
            if (this.isSearch) {
      ...
      fetchData(userName, beginDate, endDate, billNo, pageIndex, pageSize) {
        ...
        let status = [{Status0: 'C'}, {Status1: 'D'}]
        const billTypeID = '5acad595ad0efe'
      ...
      checkAll() {
        if (this.isAllSel === false) {
          for (let i = 0; i < this.dataList.length; i++) {
            ...
            this.checkedMaterialList[this.dataList[i].BillNo] = this.dataList[i]
          }
          this.isAllSel = true
        } else {
          for (let i = 0; i < this.dataList.length; i++) {
            this.dataList[i].checked = false
            delete this.checkedMaterialList[this.dataList[i].BillNo]
          }
          this.isAllSel = false
        }
      },
      ...
      deleteBill() {
        ...
        if (keys.length === 0) {
          ...
          if (this.checkedMaterialList[keys[0]].DocumentStatus === 'B') {
            this.$vux.alert.show({
              title: '警告',
              content: '提交状态单据 不可删除!'
            })
