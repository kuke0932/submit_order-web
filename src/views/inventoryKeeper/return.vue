  ...
  <div>
    <div style="position: fixed; top: 46px; width: 100%;z-index: 999">
      ...
      <tab style="position: fixed; top: 170px; width: 100%;">
        <tab-item selected @on-item-click="tabOnItemClick">待收货</tab-item>
      ...
      <div class="select_all" style="position: fixed; top: 213px; width: 100%;">
        ...
        <x-button style="margin-top: 0;" v-if="currentTab == 0" mini @click.native="batchAudit">收货</x-button>
        <x-button style="margin-top: 0;" v-if="currentTab == 0" mini @click.native="batchUnAudit">撤销</x-button>
        <x-button style="margin-top: 0;" v-if="currentTab == 0" mini @click.native="deleteBill">删除</x-button>
        <x-button style="margin-top: 0;" v-if="currentTab == 1" mini @click.native="batchUnAudit">反审核</x-button>
      </div>
    </div>
              ...
              <flexbox-item class="details_right">
                <router-link :to="{name:'inventoryKeeperDetails', params: {borrowDetails: item, fromType: 0}}">
                  ...
                  <flexbox>
                    ...
                    <flexbox-item>状态：{{item.DocumentStatus == 'A' ? '已保存' : item.DocumentStatus == 'D' ? '反审核' : item.DocumentStatus == 'B' ? '待审核'  :'已审核'}}</flexbox-item>
                  </flexbox>
                </router-link>
              </flexbox-item>
            </flexbox>
...
<script>
  ...
  import { listOfReturnMaterials as retReturnList } from '@/api/return'
  import { audit, unAudit, removeReturn } from '@/api/base'
  import Vue from 'vue'
  ...
  export default {
    ...
    methods: {
      ...
      fetchData(currentTab, userName, beginDate, endDate, billNo, pageIndex, pageSize) {
        ...
        let status = currentTab === 0 ? [{Status0: 'B', Status1: 'D'}] : [{Status0: 'C'}]
        const billTypeID = '5a9ddf63999b03'
        return retReturnList(deptID, organizationID, userId, userName, status, beginDate, endDate, billNo, billTypeID, pageIndex, pageSize)
      },
      onCalendarChange() {
