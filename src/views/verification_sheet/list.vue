  ...
  <div style="height:100%;">
    <view-box ref="viewBox">
      <x-header :left-options="{preventGoBack: true}"
                @on-click-back="back" slot="header" >已使用
      </x-header>
      <div class="container">
        <div style="position: fixed; top: 46px; width: 100%;z-index:999">
          ...
          <!--<tab style="position: fixed; top: 90px; width: 100%;">
            <tab-item @on-item-click="tabOnItemClick">已保存</tab-item>
            <tab-item selected @on-item-click="tabOnItemClick">已提交</tab-item>
          ...
          <div class="select_all" v-if="currentTab != 2" style="position: fixed; top: 134px; width: 100%;">
            ...
            &lt;!&ndash;<x-button style="margin-top: 0;" v-if="currentTab == 0" mini @click.native="edit">编辑</x-button>&ndash;&gt;
        ...
        <div :style="currentTab == 2 ? 'margin-top: 90px; width: 100%;height:' + (viewHeight - 143) + 'px' :
              ...
              <div class="infor_item" v-for="(item, index) in (isSearch ? searchList : dataList[currentTab])" v-bind:key="index">
                <flexbox class="details">
                  ...
                  <flexbox-item class="details_right">
                    <router-link :to="{path:'/verification_sheet/detail', query: {billNo: item.BillNo, currentType: 1}}">
                      <flexbox>
                        <flexbox-item>{{item.BillNo}}</flexbox-item>
                        <flexbox-item>{{item.Date}}</flexbox-item>
                      </flexbox>
                      <flexbox>
                        <flexbox-item>{{item.Specification.substr(0,11)}}</flexbox-item>
...
//      this.fetchData(0, '', 1, 10)
    ...
    methods: {
      refresh(currentTab) {
        return (done) => {
          this.tabOnItemClick(currentTab, done)
        }
      },
      infinite(done) {
      ...
      fetchData(currentTab, billNo, pageIndex, pageSize) {
        ...
        let status = [{Status0: 'C'}]
        return getIsReturnMaterial(organizationID, userId, status, this.date[0] || '', this.date[1] || this.date[0] || '', billNo, pageIndex, pageSize)
      },
      onSubmit() {
