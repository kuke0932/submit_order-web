<template>
  <div style="height:100%;">
    <view-box ref="viewBox">
      <x-header :left-options="{preventGoBack: true}" @on-click-back="back"  slot="header">收款单
                ...
                <flexbox class="details">
                  <flexbox-item :span="2" class="details_left">
                    ...
                    <flexbox-item class="details_right">
                      <router-link :to="{path: '/collection/collectionDetail?t=4', query: {fid: item.fid}}">
                        <flexbox>
...
<script>
  ...
  export default {
    ...
    methods: {
      addBill() {
        this.$router.push({ name: 'collectionBill', query: { t: 4, noPgd: true } })
      },
      refresh(done) {
        this.pageIndex = 1
        this.pageSize = 10
      ...
      fetchData() {
        return getList(this.orgID, this.empID, this.searchContent, this.pageIndex, this.pageSize)
      },
      onSubmit() {
        this.refresh()
      },
      ...
      /* checkAll() {
        if (this.isAllSel === false) {
          for (let i = 0; i < this.collectionList.length; i++) {
            this.collectionList[i].checked = true
            this.checkList[this.collectionList[i].BillNo] = this.collectionList[i]
          }
          this.isAllSel = true
        } else {
          for (let i = 0; i < this.collectionList.length; i++) {
            this.collectionList[i].checked = false
            delete this.checkList[this.collectionList[i].BillNo]
          }
          this.isAllSel = false
        }
      }, */
      ...
      toCollection() {
        ...
        if (keys.length !== 1) {
          ...
          toCollection(fid, this.orgID).then(resp => {
            console.log(resp)
            const data = resp[0]
            if (data.StatusCode) {
